...
function render(d) {
  ...
  const lateNote = lateDow && lateDow[1] > d.range.start
    ? `${lateDow[0]}days are missing from the start of the archive &mdash; the first one held is ${longDate(lateDow[1])}, so ${lateDow[0]}day notices before that date are absent. `
    : '';
  ...
  out.push(`<div class="caveat">
    <strong>Read this first.</strong> This archive holds <span class="num">${n0(cov.publication_days)}</span> publication days
    out of <span class="num">${n0(cov.weekdays_in_range)}</span> weekdays between ${longDate(d.range.start)} and ${longDate(d.range.end)}.
    ${lateNote}${outageNote}The gaps are not spread evenly, so raw monthly counts cannot be compared to each other.
  ...
  out.push(`<div class="toc"><h3>What the year shows</h3><ol>
    <li><a href="#cliff">Everything happens in June</a></li>
    <li><a href="#renewal">Renewals move more money than competition does</a></li>
    <li><a href="#median">The typical award got bigger in June and stayed bigger</a></li>
    <li><a href="#dycd">Where the non-competitive money goes</a></li>
    ...
    <li><a href="#swap">A section was renamed, and the numbers fell out of it</a></li>
    ...
    <li><a href="#bids">The bid window never moved</a></li>
  ...
  out.push(`<section class="finding" id="cliff">
    ...
    <h2>Everything happens in June</h2>
    <p class="standfirst">New York City's fiscal year ends June 30. Most of the year's contracting business is published in the few weeks on either side of that date.</p>
    <p>Through the spring the city published about <span class="num">${n1(baseline)}</span> contract awards on a typical publication day. In ${peakRate.label} the rate hit <span class="num">${n1(peakRate.awards_per_day)}</span>, ${n1(peakRate.awards_per_day / baseline)} times the earlier pace. It has not come back down.</p>
    ...
    <p>The window runs from ${longDate(b.from)} to ${longDate(b.to)}. In those <span class="num">${n0(b.days)}</span> publication days the city put out <span class="num">${n0(b.awards)}</span> awards worth <span class="num">${money(b.value)}</span> &mdash; <span class="num">${pct(boundaryCountShare)}</span> of every award in the archive and <span class="num">${pct(boundaryValueShare)}</span> of every published dollar. <span class="num">${n0(b.renewals)}</span> of them, ${pct(b.renewals / b.awards)}, were renewals of contracts the city already held.</p>
    <p>The titles show it too. <span class="num">${n0(d.fiscal_year.tagged_awards)}</span> awards worth <span class="num">${money(d.fiscal_year.tagged_value)}</span> name the coming fiscal year in the title, and they bunch in the same weeks: ${d.fiscal_year.by_month.filter(x => x.count > 0).map(x => `${short(x)} ${n0(x.count)}`).join(', ')}.</p>
    <div class="pullout">Nearly a third of the year's contracting business, by count and by dollar, moves in the four weeks around June 30.</div>
  ...
  out.push(`<section class="finding" id="renewal">
    ...
    <p>Renewals are also what makes June look the way it does. Their monthly count runs ${M.slice(0, 3).map(m => `${short(m)} ${n0(m.renewals)}`).join(', ')} early in the year, then spikes to <span class="num">${n0(Math.max(...M.map(m => m.renewals)))}</span> in ${short(M.reduce((a, x) => x.renewals > a.renewals ? x : a))}.</p>
    ...
    <p>None of this is irregular. Renewing a multi-year human-services contract is ordinary, and the notice runs exactly as the rules require. But it means most of the dollars in the paper this year were decided in an earlier year, and the renewal notice is the only public-comment window on them.</p>
  </section>`);
  ...
  out.push(`<section class="finding" id="median">
    ...
    <p>The size mix shows what happened. Small awards did not go away: awards under $100,000 ran <span class="num">${n1(smallFirst)}</span> a day in ${first.label} and <span class="num">${n1(smallLast)}</span> a day in ${last.label}. The middle and upper bands did. Awards between $1 million and $10 million went from <span class="num">${n1(midFirst)}</span> a day to <span class="num">${n1(midLast)}</span> a day, peaking at <span class="num">${n1(Math.max(...M.map(m => m.bands.m1_10 / m.days)))}</span> a day in ${short(M.reduce((a, x) => (x.bands.m1_10 / x.days) > (a.bands.m1_10 / a.days) ? x : a))}.</p>
  ...
  out.push(`<section class="finding" id="dycd">
    ...
    <h2>Where the non-competitive money goes</h2>
    <p class="standfirst">The Department of Youth and Community Development published <span class="num">${n0(yFirst.awards)}</span> contract awards in ${yFirst.label} and <span class="num">${n0(yPeak.awards)}</span> in ${yPeak.label}.</p>
    <p>Across the archive the agency accounts for <span class="num">${n0(y.total_awards)}</span> awards worth <span class="num">${money(y.total_value)}</span>. The method is unusual. <span class="num">${n0(y.negotiated_awards)}</span> of those awards, worth <span class="num">${money(y.negotiated_value)}</span>, were negotiated acquisitions, the route agencies use when a normal competition is impractical. There were <span class="num">${n0(y.citywide_negotiated)}</span> negotiated acquisitions citywide in the whole archive. One agency accounts for <span class="num">${pct(y.negotiated_awards / y.citywide_negotiated)}</span> of them.</p>
    ...
    <p>The money is spread thin and wide. <span class="num">${n0(y.negotiated_vendors)}</span> separate organizations, most of them community nonprofits, share those negotiated awards. The programs behind them are the familiar ones.</p>
    ...
    <p>So a large share of the city's after-school, community-center and summer-jobs money is moving through a non-competitive route, at high volume, to a long list of small providers. The City Record does not say why. It says only that the pattern started in the spring and has not stopped.</p>
  ...
  out.push(`<section class="finding" id="compression">
    ...
    <p class="standfirst">Citywide charts smooth this out. Agency by agency, one month often carries almost all of the year's published dollars.</p>
    ...
    <p>${compressed[0] ? `${esc(compressed[0].name)} tops the list at <span class="num">${pct(compressed[0].peak_share, 1)}</span> of <span class="num">${money(compressed[0].value)}</span> in ${compressed[0].peak_month.label}, but that is about size, not timing: two very large ${compressed[0].peak_month.label} awards swamp everything else it published all year.` : ''} ${bulk ? `${esc(bulk.name)} is the better example. It put <span class="num">${pct(bulk.peak_share, 1)}</span> of its published dollars into ${bulk.peak_month.label} across <span class="num">${n0(bulk.by_month[bulk.peak_month.month].count)}</span> separate awards, which is calendar compression rather than one big contract.` : ''}</p>
    <p>Either way the consequence is the same for anyone following a single agency. Miss one month of the City Record and you can miss most of that agency's year.</p>
  </section>`);
  ...
  out.push(`<section class="finding" id="swap">
    ...
    <h2>A section was renamed, and the numbers fell out of it</h2>
    <p class="standfirst">Some of what changed this year is about the paper, not the city. One section emptied out in the winter and another filled up in its place.</p>
    <p>Through ${oldPeak.label} the archive's Contract Award Hearings section carried <span class="num">${n0(oldPeak.award_hearings)}</span> notices. After that it holds <span class="num">${n0(oldAfter)}</span> in total. Starting ${longDate(sw.public_comment_first)} a section called Public Comment on Contract Awards appears, and it has since carried <span class="num">${n0(newTotal)}</span> notices . It does the same job, opening a proposed award to public comment, under a new heading.</p>
    ...
    <p>Something was lost in the move. Notices in the old section came through with the standard procurement fields. The records in the new one arrive with no vendor name and no contract amount at all &mdash; the fields are absent from the row, not blank. More awards are open to public comment in the record, and less is known about them.</p>
  ...
  out.push(`<section class="finding" id="eo">
    ...
    <p class="standfirst">The change of administration shows up in the paper as a number running backwards.</p>
    ${highNum.length && resets.length ? `<p>On ${longDate(highNum[highNum.length - 1].date)} the paper published emergency executive orders numbered in the ${Math.floor(parseFloat(highNum[highNum.length - 1].number) / 100) * 100}s &mdash; the end of the previous administration's sequence. Days later, on ${longDate(resets[0].date)}, it published Executive Order No. 1. Both the regular and the emergency counters restart inside the same two weeks.</p>` : ''}
  ...
  out.push(`<section class="finding" id="emergency">
    ...
    <p class="standfirst">Emergency procurement is rare in the archive &mdash; <span class="num">${n0(em.length)}</span> awards worth <span class="num">${money(emTotal)}</span> &mdash; and most of it is either weather or a building coming down.</p>
    ${snow.length ? `<p>The first is weather. <span class="num">${n0(snow.length)}</span> emergency awards worth <span class="num">${money(snow.reduce((s, e) => s + e.value, 0))}</span> cover snow operations, snow transportation and warming buses, and they are published between ${longDate(snow[0].date)} and ${longDate(snow[snow.length - 1].date)} &mdash; well after the season they belong to. The largest single one is ${money(Math.max(...snow.map(e => e.value)))}.</p>` : ''}
    ...
    <p>With emergency awards the notice comes after the fact. The work is done and the money committed by the time it runs, and the City Record is the disclosure. So the list doubles as a record of what went wrong in the city.</p>
  </section>`);
  ...
  out.push(`<section class="finding" id="bids">
    ...
    <h2>The bid window never moved</h2>
    <p class="standfirst">Volume changed, award size changed, method changed. The time a vendor gets to answer a solicitation did not.</p>
    ...
    <p>Short windows do exist. <span class="num">${pct(bw.short_share, 1)}</span> of dated solicitations give two weeks or less, and a handful run within days of their own deadline &mdash; ${bw.very_short.length ? `the shortest in the archive is ${n0(bw.very_short[0].days)} days, on ${esc(bw.very_short[0].agency)}'s "${esc((bw.very_short[0].title || '').slice(0, 56))}" notice` : ''}. Some of those are real deadlines set close in. Others are almost certainly a notice published late against a deadline set earlier. The City Record cannot tell the two apart, and neither can this page.</p>
  </section>`);
  ...
  out.push(`<section class="finding" id="themes">
    ...
    <p class="standfirst">Five recurring subjects are big enough in the archive to chart. Two of them are moving.</p>
    ${themeRows.map(t => {
    ...
    <p>Shelter and homeless services is the biggest of them by dollars &mdash; <span class="num">${money(shelter.series.reduce((s, x) => s + x.value, 0))}</span> across <span class="num">${n0(themeTotal(shelter))}</span> notices &mdash; and unlike most categories it does not drop off after the fiscal-year line. Its published award value in ${shelter.series[shelter.series.length - 1].label} alone is <span class="num">${money(shelter.series[shelter.series.length - 1].value)}</span>.</p>
    <p>Immigrant and low-wage-worker legal services is the sharpest late move, though the numbers are small: <span class="num">${n0(themeTotal(legal))}</span> notices in the whole archive, <span class="num">${n0(legal.series[legal.series.length - 1].notices)}</span> of them in ${legal.series[legal.series.length - 1].label} and <span class="num">${n0(legal.series[legal.series.length - 2] ? legal.series[legal.series.length - 2].notices : 0)}</span> in ${legal.series[legal.series.length - 2] ? legal.series[legal.series.length - 2].label : ''}. They come from the Department of Social Services and the Department of Youth and Community Development. ${legal.top && legal.top.length >= 2 ? `The two largest, both published on ${longDate(legal.top[0].date)}, went to ${esc(legal.top[0].vendor)} (<span class="num">${money(legal.top[0].value)}</span>) and ${esc(legal.top[1].vendor)} (<span class="num">${money(legal.top[1].value)}</span>).` : ''}</p>
    <p>Two things are notable for being absent. Across ${n0(d.coverage.publication_days)} publication days, ${themeThin.map(t => `<span class="num">${n0(themeTotal(t))}</span> ${themeTotal(t) === 1 ? 'notice mentions' : 'notices mention'} ${t.label.toLowerCase()}`).join(', and ')}. Whatever the city is doing on either front, it is not arriving through published contract notices in this period.</p>
  </section>`);
  ...
  out.push(`<section class="finding" id="limits">
    ...
    <p>Four limits sit under everything above, and none of them can be coded around.</p>
    ...
    <p><strong>The record shows the notice, not the reason.</strong> The City Record says an agency used a negotiated acquisition. It does not say why, whether the determination held up, or whether anyone objected. Those answers live in agency determinations, Comptroller registration and the Procurement Policy Board record.</p>
    <p><strong>The paper itself changes.</strong> Finding 07 is the example. A section can be renamed and its machine-readable fields can disappear with no announcement anywhere in the data. Any trend line that crosses a change like that is measuring the paper as much as the city.</p>
