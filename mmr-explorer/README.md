...
## Reader navigation and comparisons

The default landing view is **Explore topics**. Existing `#board`, agency, search, ratio and indicator links continue to work. Six curated topics contain twelve questions; `data/guide.json` holds their exact indicator IDs, scope notes, and attributed chapter explanations. This file is reviewed editorial content, not generated by the numerical pipeline. Check the indicator definitions and linked PDF pages before changing it.

`explore.js` implements topic navigation, search synonyms, related indicators, comparison selection (up to four), reading boxes and CSV exports. The selected IDs are encoded in the `compare` query field in the URL. Comparison charts share report years and retain their individual units and vertical scales. Different reporting periods produce a warning; the comparison does not claim that different series form a cohort or causal relationship.

Related measures use curated topic relationships where available. Otherwise they are explicitly labeled as other measures under the same agency goal. Reading boxes distinguish selected-year change from the FY2026 target judgment. Curated agency explanations are attributed and page-linked; uncurated measures link to the source without inventing an explanation. Known definition breaks are marked on charts and interrupt connecting lines.

CSV exports include all report years for selected indicators, numeric and display values, definitions, reporting periods, source URLs, original values where the report revised Open Data history, desired directions, flags and the selected comparison years. Clock readings are exported numerically in decimal major units (for example, minutes) and also as formatted clock values. Missing observations remain empty. Text that could be interpreted as a spreadsheet formula is escaped.
