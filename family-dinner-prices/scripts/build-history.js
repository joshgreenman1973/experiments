...
const closedIds = new Set(registry.filter(r => r.status === 'closed').map(r => r.id));
console.log(`Active: ${registry.length - closedIds.size} | Closed: ${closedIds.size}`);
...
for (const snap of snapshots) {
  // City-wide average (exclude closed restaurants)
  const allPrices = Object.entries(snap.prices)
    .filter(([id]) => !closedIds.has(id))
    .map(([, p]) => totalWithTip(p.price));
  trends.citywide.push(Math.round(allPrices.reduce((a, b) => a + b, 0) / allPrices.length * 100) / 100);

  // Borough averages (exclude closed)
  for (const borough of boroughs) {
    const ids = registry.filter(r => r.borough === borough && !closedIds.has(r.id)).map(r => r.id);
  ...
  // Cuisine group averages (exclude closed)
  for (const [group, datasets] of Object.entries({
    ...
    const ids = registry.filter(r => datasets.includes(r.dataset) && !closedIds.has(r.id)).map(r => r.id);
    const prices = ids.filter(id => snap.prices[id]).map(id => totalWithTip(snap.prices[id].price));
    const avg = prices.length ? Math.round(prices.reduce((a, b) => a + b, 0) / prices.length * 100) / 100 : null;
...
const closures = registry
  ...
  .map(r => ({
    ...
    closedDate: r.closedDate || 'unknown',
    reason: r.closureReason || 'unknown',
  }));
