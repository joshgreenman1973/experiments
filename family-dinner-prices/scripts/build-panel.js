...
function buildComponents(o) {
  ...
  if (o.mealType === 'pizza') {
    ...
    return { components: [{ role: 'Large pizza', item: z.item, base: z.price, qty: 1 }], overCeiling: z.price > 35 };
  }
...
for (const o of pinned) {
  ...
  panel[o.id] = {
    ...
    mealType: o.mealType, components: built.components, beveragePrice: BEV, beverageQty: BEVQTY,
    baselineBill: billOf(built.components), overCeiling: built.overCeiling,
  };
  kept++; byType[o.mealType] = (byType[o.mealType] || 0) + 1;
}

const out = {
  meta: {
    baselineDate, beveragePrice: BEV, beverageQty: BEVQTY, tax: TAX, tip: TIP,
    mealModels: { pizza: '1 large pizza + 2 drinks', chinese: '2 shared dishes + egg rolls + 2 drinks', standard: '2 adult entrees + 2 kid portions + 2 drinks' },
